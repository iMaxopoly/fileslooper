...
class TwoddlRequestSpider(scrapy.Spider):
    __domain = "www.filesloop.com"
    __protocol = "https"
    __baseURL = __protocol + "://" + __domain
    __driver = "2ddl"
    __write_dir = "debug"
    __write_filename = "twoddl_url.txt"
    __search_term = ""
    __licensor_name = ""
    ...
    def __init__(self, search_term, licensor_name, time_stamp, **kwargs):
        ...
        super(TwoddlRequestSpider, self).__init__(**kwargs)
    ...
    def parse_exit_filesloop_page(self, response):
        ...
        for redir_link in response.request.meta['redirect_urls']:
            ind = redir_link.find("http://twoddl.tv/")
            if ind is not -1:
                searchserver_link = redir_link[ind:]
