...
class AllucRequestSpider(scrapy.Spider):
    __domain = "www.filesloop.com"
    __protocol = "https"
    __baseURL = __protocol + "://" + __domain
    __driver = "alluc"
    __write_dir = "debug"
    __write_filename = "alluc_url.txt"
    __search_term = ""
    __licensor_name = ""

    __alluc_request_url = "https://www.filesloop.com/fileslist/get/" + __driver
    __alluc_request_content_type = {
        "Content-Type": "application/json",
        "Requested-With-AngularJS": "true"
    }
    __alluc_request_method = "POST"
    __alluc_request_page_number = 0
    ...
    def __init__(self, search_term, licensor_name, time_stamp, **kwargs):
        ...
        print "Setup with", self.__search_term, self.__licensor_name
        super(AllucRequestSpider, self).__init__(**kwargs)

    def start_requests(self):
        yield Request(
            url=self.__alluc_request_url,
            method=self.__alluc_request_method,
            body=json.dumps({
                "search": self.__search_term, "server": "all", "order": "relevance", "filesize": "all", "ext": "all",
                "page": self.__alluc_request_page_number, "driver": self.__driver
            }),
            headers=self.__alluc_request_content_type
        )
    ...
    def parse(self, response):
        ...
        self.__alluc_request_page_number += 1
            ...
            body=json.dumps(
                {
                    ...
                    "ext": "all", "page": self.__alluc_request_page_number, "driver": self.__driver
                }),
            headers=self.__alluc_request_content_type,
            dont_filter=True,
            callback=self.parse
