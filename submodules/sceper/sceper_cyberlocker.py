...
class SceperCyberlockerSpider(scrapy.Spider):
    __domain = "sceper.ws"
    __protocol = "http"
    __baseURL = __protocol + "://" + __domain
    __write_dir = "debug"
    __write_filename = "sceper_cyberlocker_url.txt"
    __read_filename = "debug/{0}/sceper_url.txt"
    __timestamp = ""
    ...
    def __init__(self, search_term, licensor_name, time_stamp, **kwargs):
        ...
        super(SceperCyberlockerSpider, self).__init__(**kwargs)
    ...
    def parse(self, response):
        ...
        cyberlocker_links = response.xpath("//a/@href").extract()
        for cyberlocker_link in cyberlocker_links:
            if "http://rockfile.eu" in cyberlocker_link or \
                            "http://hugefiles.net" in cyberlocker_link or \
                            "http://ul.to" in cyberlocker_link or \
                            "https://userscloud.com" in cyberlocker_link or \
                            "http://rapidgator.net/file" in cyberlocker_link or \
                            "http://nitroflare.com/view" in cyberlocker_link or \
                            "http://rapidgator.net/folder" in cyberlocker_link or \
                            "http://uploadrocket.net" in cyberlocker_link or \
                            "http://uploading.site" in cyberlocker_link or \
                            "https://openload.co" in cyberlocker_link or \
                            "http://www.filefactory.com/file" in cyberlocker_link or \
                            "http://uploaded.net/file" in cyberlocker_link or \
                            "http://kingfiles.net/" in cyberlocker_link:
