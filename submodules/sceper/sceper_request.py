...
class SceperRequestSpider(scrapy.Spider):
    __domain = "www.filesloop.com"
    __protocol = "https"
    __baseURL = __protocol + "://" + __domain
    __driver = "sceper"
    __write_dir = "debug"
    __write_filename = "sceper_url.txt"
    __search_term = ""
    __licensor_name = ""
    ...
    def __init__(self, search_term, licensor_name, time_stamp, **kwargs):
        ...
        super(SceperRequestSpider, self).__init__(**kwargs)
    ...
    def parse(self, response):
        ...
        for f in files:
            ...
            request = Request(url=response.urljoin(file_url), meta={'item': item},
                              callback=self.parse_exit_filesloop_page)
            yield request
